  ...
  function WomenShoes() {
    ...
    useEffect(() => {
      fetch('http://localhost:5000/products')
        .then((res) => res.json())
        .then((data) => {
          const zenskiProizvodi = data.filter((product) => product.gender === 'zenske');
          setProducts(zenskiProizvodi);
          setLoading(false);
        })
        .catch((error) => {
          console.error('Greska pri ucitavanju zenskih patika:', error);
          setLoading(false);
        });
    }, []);
      ...
      <div className="page-container">
        <section>
          <div className="mb-6">
            ...
            <h2 className="text-3xl font-bold text-slate-950">Zenske patike</h2>
          </div>
              ...
              {products.map((product) => (
                ...
                >
                  <div className="mb-5 flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs uppercase tracking-[0.28em] text-cyan-700">Zenske</p>
                      <h3 className="mt-2 text-2xl font-bold text-slate-950">{product.name}</h3>
                    </div>
                    <span className="text-nowrap rounded-full bg-amber-400 px-3 py-1 text-sm font-bold
  text-slate-950">
                      {product.price} RSD
                    </span>
                  </div>
